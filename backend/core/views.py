...
class CertificateDownloadView(APIView):
    ...
    def get(self, request, module_id):
        ...
        # Use landscape for a real certificate feel
        ...
        p.drawCentredString(badge_x + 0.75 * inch, badge_y + 4, f"SKILL LEVEL: {(user.level or 'PRO').upper()}")
        ...
        p.drawCentredString(width / 2.0, height - 3.6 * inch, f"{user.first_name} {user.last_name}".upper() if user.first_name else user.username.upper())
        ...
        p.drawCentredString(width / 2.0, height - 5.0 * inch, certificate.module)
        ...
        p.drawString(1.1 * inch, 1.3 * inch, f"Issued on: {certificate.issued_at.strftime('%B %d, %Y')}")

        # Center: QR Code
        qr_code = qr.QrCodeWidget(f"https://python-edition.vercel.app/verify/{certificate.id}")
        ...
        p.setFont("Courier", 7)
        p.drawCentredString(width/2.0, 1.1 * inch, f"ID: {certificate.id}")
